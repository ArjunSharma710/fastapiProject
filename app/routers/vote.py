...
def vote(vote:schemas.Vote, db:Session=Depends(get_db),current_user:int= Depends(oauth2.get_current_user)):
    ...
    if (vote.dir==1):
        ...
        if found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail=f"user with user id{current_user.id} has already voted on this post with id{vote.post_id}")
